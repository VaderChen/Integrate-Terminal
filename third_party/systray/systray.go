...
// SetReopenHandler registers a callback for platform reopen events, such as
// clicking the app icon while the app is already running on macOS.
func SetReopenHandler(onReopen func()) {
	if onReopen == nil {
		systrayReopen = nil
		return
	}
	systrayReopen = onReopen
}
...
func systrayHandleReopen() {
	if systrayReopen != nil {
		go systrayReopen()
	}
}
