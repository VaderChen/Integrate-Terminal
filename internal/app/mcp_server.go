...
import (
	...
	appversion "IntegTERM/internal/version"
	"github.com/modelcontextprotocol/go-sdk/mcp"
)

// A 4 MiB inline string can expand sixfold when escaped in JSON.
const mcpMaxFrameBytes = 26 << 20
...
func (layer *mcpVirtualLayer) newServer(contract mcpContract) *mcp.Server {
	server := mcp.NewServer(&mcp.Implementation{
		...
		SetCacheable: func(_ context.Context, _ mcp.Request, value *mcp.Cacheable) {
			value.TTLMs = 0
...
func mcpServerInstructions(contract mcpContract) string {
	...
	return strings.Join([]string{
		...
		"If saved-site discovery reports a locked or unauthorized Keychain, authorize access in the desktop app and retry. RAM operations and the root listing remain usable. Local stdio never opens an authorization dialog; a busy data store is reported after a bounded wait.",
		...
		"The RAM workspace holds up to 32 MiB and 4096 entries, with a separate 32 MiB chunk staging budget and at most 64 pending writes. Paths are limited to 4096 bytes and 64 components; a process can mount up to 64 sites.",
...
func (layer *mcpVirtualLayer) addNetworkFeatures(server *mcp.Server) {
	endpoints := buildRESTEndpointDocs("")
...
func (a *App) RunMCPStdio(ctx context.Context) error {
	return newMCPVirtualLayer(a).newServer(mcpContractLocal).Run(ctx, &mcp.StdioTransport{MaxLineLength: mcpMaxFrameBytes})
}
...
func newMCPStreamableHTTPHandler(server *mcp.Server) http.Handler {
	return mcp.NewStreamableHTTPHandler(func(*http.Request) *mcp.Server {
		...
		Stateless:                    true,
		JSONResponse:                 true,
		MaxRequestBodyBytes:          mcpMaxFrameBytes,
...
func mcpInputSchema(endpoint endpointDoc) map[string]any {
	...
	for name, example := range endpoint.Request {
		properties[name] = mcpExampleSchema(example)
	}
	...
	if required := pathParameterNames(endpoint.Path); len(required) > 0 {
		schema["required"] = required
	}
	if endpoint.Method == http.MethodPut && endpoint.Path == "/api/config" {
...
func mcpExampleSchema(value any) map[string]any {
	schema := map[string]any{"description": fmt.Sprintf("Example: %v", value)}
	...
	case map[string]any:
		...
		for key, child := range value {
			properties[key] = mcpExampleSchema(child)
		}
