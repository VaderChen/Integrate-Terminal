...
const (
	mcpVFSMaxNodes         = 4096
	mcpVFSMaxPathBytes     = 4096
	mcpVFSMaxDepth         = 64
	mcpVFSMaxPendingWrites = 64
)

type mcpVFS struct {
	remoteOpsMu  sync.Mutex
	mu           sync.RWMutex
...
func (vfs *mcpVFS) writeBytes(path string, data []byte, overwrite bool, maxFileBytes int64) (mcpVFSItem, error) {
	...
	reserved := 0
	if _, exists := vfs.nodes[relativePath]; !exists {
		reserved = 1
	}
	if err := vfs.ensureDirectoryLocked(mcpVFSParent(relativePath), reserved); err != nil {
		return mcpVFSItem{}, err
	}
...
func (layer *mcpVirtualLayer) writeVirtualChunk(input mcpVFSWriteChunkInput) (mcpVFSWriteChunkOutput, error) {
	...
	chunk, err := decodeMCPVFSContent(input.Content, input.Encoding, mcpVFSMaxWriteChunk)
	if err != nil {
		return mcpVFSWriteChunkOutput{}, err
	}
	...
	if !exists && len(vfs.chunkWrites) >= mcpVFSMaxPendingWrites {
		vfs.chunkMu.Unlock()
		return mcpVFSWriteChunkOutput{}, fmt.Errorf("too many pending chunk writes; finish or restart an existing destination")
	}
	if len(chunk) == 0 && !input.Final {
		vfs.chunkMu.Unlock()
		return mcpVFSWriteChunkOutput{}, fmt.Errorf("non-final chunks must not be empty")
	}
	if input.Offset == 0 {
		if exists {
...
func (vfs *mcpVFS) mkdir(path string) (mcpVFSItem, error) {
	...
	if err := vfs.ensureDirectoryLocked(relativePath, 0); err != nil {
		return mcpVFSItem{}, err
	}
	return mcpVFSItemFromNode(relativePath, vfs.nodes[relativePath]), nil
}
...
func (vfs *mcpVFS) rename(oldPath string, newPath string) (mcpVFSItem, error) {
	...
	// Check every descendant before creating parents or moving any nodes.
	for nodePath := range vfs.nodes {
		if strings.HasPrefix(nodePath, oldRelativePath+"/") {
			if _, err := normalizeMCPVFSPath(newRelativePath + strings.TrimPrefix(nodePath, oldRelativePath)); err != nil {
				return mcpVFSItem{}, err
			}
		}
	}
	if err := vfs.ensureDirectoryLocked(mcpVFSParent(newRelativePath), 0); err != nil {
		return mcpVFSItem{}, err
	}
...
// Check the entire parent chain before mutating it. A file can never be an ancestor.
func (vfs *mcpVFS) ensureDirectoryLocked(path string, reservedNodes int) error {
	missing := []string{}
	for current := path; current != ""; current = mcpVFSParent(current) {
		node, exists := vfs.nodes[current]
		if exists && !node.directory {
			return fmt.Errorf("virtual ancestor is a file: %s", current)
		}
		if !exists {
			missing = append(missing, current)
		}
	}
	if len(vfs.nodes)+len(missing)+reservedNodes > mcpVFSMaxNodes {
		return fmt.Errorf("workspace exceeds %d entries", mcpVFSMaxNodes)
	}
	now := time.Now().UTC()
	for _, current := range missing {
		vfs.nodes[current] = mcpVFSNode{directory: true, modified: now}
	}
	return nil
}
...
func addMCPVFSFeatures(server *mcp.Server, layer *mcpVirtualLayer) {
	...
	mcp.AddTool(server, &mcp.Tool{
		...
		count, err := layer.mcpRemoteSiteCount()
		if err != nil {
			return nil, mcpVFSWorkspaceOutput{}, err
		}
		info.RemoteSiteCount = count
		return nil, info, nil
	})
...
func decodeMCPVFSContent(content, encoding string, maxBytes int64) ([]byte, error) {
	// Bound the input before allocating its decoded copy (base64 may otherwise
	// allocate from an arbitrarily large request before the file limit is checked).
	encodedLimit := maxBytes
	if strings.EqualFold(strings.TrimSpace(encoding), "base64") {
		encodedLimit = ((maxBytes + 2) / 3) * 4
	}
	if int64(len(content)) > encodedLimit {
		return nil, fmt.Errorf("content exceeds %d decoded bytes", maxBytes)
	}
...
func normalizeMCPVFSPath(value string) (string, error) {
	if len(value) > mcpVFSMaxPathBytes*3+len(mcpVFSRootURI) {
		return "", fmt.Errorf("virtual path is too long")
	}
	...
	if strings.Contains(value, "://") {
		...
		if !strings.EqualFold(parsed.Scheme, "integterm-vfs") || !strings.EqualFold(parsed.Host, "workspace") || parsed.RawQuery != "" || parsed.ForceQuery || parsed.Fragment != "" || parsed.User != nil {
	...
	// Validate again after URL decoding so %00 and encoded control bytes cannot
	// reach remote protocol commands.
	for _, r := range value {
		if r < 32 || r == 127 {
			return "", fmt.Errorf("virtual path contains a control character")
		}
	}
	if !utf8.ValidString(value) || len(value) > mcpVFSMaxPathBytes || strings.Count(value, "/") >= mcpVFSMaxDepth {
		return "", fmt.Errorf("virtual path exceeds the length or depth limit")
	}
	if strings.Contains(value, "\\") {
